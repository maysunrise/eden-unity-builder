...
public class Chunk : MonoBehaviour
{
    ...
    private bool _isNeedsRefreshCollider = false;

    private MeshData _meshData;

    private MeshData _meshDataCollision;

    public bool isLoaded = false;

    public bool isConverted = false;

    public bool HasUpdatedNow;
    ...
    void Start()
    {
        ...
        GameObject liquidCollider = new GameObject();
        liquidCollider.name = "LiquidCollider";
        liquidCollider.transform.position = transform.position;
        liquidCollider.transform.parent = transform;
        liquidCollider.layer = 9;
        _liquidCollider = liquidCollider.AddComponent<MeshCollider>();
    ...
    public void RefreshAsync()
    {
        ...
        Thread collisionThread = new Thread(new ThreadStart(NeedsUpdateCollision));
        collisionThread.IsBackground = true;
        collisionThread.Priority = System.Threading.ThreadPriority.Lowest;
    ...
    public void NeedsUpdateCollision()
    {
        _meshDataCollision = GenerateCollisionMesh();
        _isNeedsRefreshCollider = true;
    }
    ...
    void BuildCollider()
    {
        ...
        _liquidCollider.sharedMesh = new Mesh();
        _meshCollider.enabled = false;
        _liquidCollider.enabled = false;
        _meshCollider.sharedMesh.vertices = _meshDataCollision.Vertices; // Collision. Maybe not use the unity physics, for perfomance?
        _meshCollider.sharedMesh.triangles = _meshDataCollision.Triangles;
        _liquidCollider.sharedMesh.vertices = _meshDataCollision.Vertices;
        _liquidCollider.sharedMesh.triangles = _meshDataCollision.TrianglesTransparent;
        _meshCollider.enabled = true;
        _liquidCollider.enabled = true;
    }
    ...
    {
        /*
        if (Vector3.Distance(_world.PlayerPosition, centeredPosition) >= _world.generationDistance+32 + _chunkSize)
        {
            if (!isDirty)
            {
        ...
        */
        ...
        if (_isNeedsRefreshCollider)
        {
            BuildCollider();
            _isNeedsRefreshCollider = false;
        }
    }
        ...
        else // Simple method
        {
            ...
            for (int x = 0; x < _chunkSize; x++)
            {
                for (int y = 0; y < _chunkSize; y++)
                {
                    for (int z = 0; z < _chunkSize; z++)
                    {
                        if (_world.ContainsIndexSegment(new Vector3Int(Position.x + x, Position.y + y, Position.z + z)))
                        {
    ...
    public MeshData GenerateCollisionMesh()
    {
        MeshBuilder builder = new MeshBuilder();
        for (int x = 0; x < _chunkSize; x++)
        {
            for (int y = 0; y < _chunkSize; y++)
            {
                for (int z = 0; z < _chunkSize; z++)
                {
                    ...
                    {
                        blocktype = GetBlock(x, y, z).BlockType;
                        isLiquid = GetBlock(x, y, z).IsLiquid();
                    }
                    if (blocktype != BlockType.Air)
                    {
                        if (!GetBlockG(x + 1, y, z).IsSolid())
                        {
                            builder.AddSimpleFace(x, y, z, 1, BlockDirection.Right, Color.black, Vector2.zero, isLiquid);
                        }
                        if (!GetBlockG(x - 1, y, z).IsSolid())
                        {
                            builder.AddSimpleFace(x, y, z, 1, BlockDirection.Left, Color.black, Vector2.zero, isLiquid);
                        }
                        if (!GetBlockG(x, y + 1, z).IsSolid())
                        {
                            builder.AddSimpleFace(x, y, z, 1, BlockDirection.Up, Color.black, Vector2.zero, isLiquid);
                        }
                        if (!GetBlockG(x, y - 1, z).IsSolid())
                        {
                            builder.AddSimpleFace(x, y, z, 1, BlockDirection.Down, Color.black, Vector2.zero, isLiquid);
                        }
                        if (!GetBlockG(x, y, z + 1).IsSolid())
                        {
                            builder.AddSimpleFace(x, y, z, 1, BlockDirection.Forward, Color.black, Vector2.zero, isLiquid);
                        }
                        if (!GetBlockG(x, y, z - 1).IsSolid())
                        {
                            builder.AddSimpleFace(x, y, z, 1, BlockDirection.Back, Color.black, Vector2.zero, isLiquid);
                        }
                    }
                }
            }
