...
public class WorldManager : MonoBehaviour // this is really old class, but i'm lazy to rewrite code :p
{
    ...
    public void InitWorlds()
    {
        ...
        /*
        FileInfo[] info2 = dir.GetFiles("*.eden2");
        foreach (FileInfo f in info2) // New world format
        {
            var world = Instantiate(World_prefab, WorldsRoot);
            CountWorlds++;
            Worlds.Add(world.GetComponent<WorldButton>());
            FileStream fileStream = new FileStream(f.ToString(), FileMode.Open);
            BinaryReader binaryReader = new BinaryReader(fileStream, Encoding.UTF8,false);
            ...
            world.GetComponent<WorldButton>().isNewFormat = true;
        }
        */
        foreach (FileInfo f in info) // Old world format
        {
            var world = Instantiate(World_prefab, WorldsRoot);
            CountWorlds++;
            Worlds.Add(world.GetComponent<WorldButton>());
