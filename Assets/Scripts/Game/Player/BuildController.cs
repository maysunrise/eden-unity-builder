...
#if UNITY_EDITOR || !PLATFORM_ANDROID // Simulating android touches for editor
...
public class BuildController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (GameUIPC.I.IsPC)
        {
            if (Input.GetKeyDown(KeyCode.Alpha1)) SelectedTool = 1;
            if (Input.GetKeyDown(KeyCode.Alpha2)) SelectedTool = 3;
            if (Input.GetKeyDown(KeyCode.Alpha3)) SelectedTool = 0;
            ...
            if (SelectedTool == 2 && Input.GetKeyDown(KeyCode.E))
            {
                InventoryBlocks.SetActive(true);
                InventoryColors.SetActive(false);
            }
            else if (SelectedTool == 3 && Input.GetKeyDown(KeyCode.E))
            {
                InventoryBlocks.SetActive(false);
                InventoryColors.SetActive(true);
            }
            MultiTouchUpdate();
        }
    ...
    public void MultiTouchUpdate()
    {
        ...
        //int layer_mask = LayerMask.GetMask("Default", "Liquid");
        for (int i = 0; i < myTouches.Length; i++)
        {
            ...
            if (SelectedTool == 2) // build
            {
                ...
                {
                    if (hit.collider != null && hit.distance < distanceRaycsat && (hit.collider.CompareTag("Chunk") || hit.collider.gameObject.layer == 9))
                    {
                        Vector3Int pos = Vector3Int.CeilToInt(hit.point + (hit.normal * 0.5f));
                        Highlighter.AddHightlight(pos - new Vector3(1f, 1f, 1f), false);
                    }
                }
                else if (myTouches[i].phase == TouchPhase.Ended || isRMB)
                {
            ...
            else if (SelectedTool == 1) // remove
            {
                ...
                else if (myTouches[i].phase == TouchPhase.Ended || isLMB)
                {
                    Vector3Int pos = Highlighter.GetHighlightPosition(myTouches[i]) + new Vector3Int(0, 1, 0);
            ...
            else if (SelectedTool == 0) // burn
            {
                ...
                else if (myTouches[i].phase == TouchPhase.Ended || isLMB)
                {
                    ...
                    if (b.isFlamming && block.isBurns == false)
                    {
                        ...
                        hit.collider.gameObject.GetComponent<Chunk>().HasUpdatedNow = true;
                    }
